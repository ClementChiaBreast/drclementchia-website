  ...
  return <header className="relative z-50 border-b hairline bg-[var(--cream)]">
    <div className="page-shell flex h-[82px] items-center justify-between">
      ...
      <nav className="hidden items-center gap-5 xl:flex">
        {links.map(([label, href]) => <Link key={href} href={href} className={`text-xs font-bold uppercase tracking-[.07em] transition-colors hover:text-[var(--coral)] ${path === href ? "border-b border-current pb-1" : ""}`}>{label}</Link>)}
      </nav>
      <Link href="/contact" className="hidden rounded-full bg-[var(--ink)] px-5 py-3 text-xs font-bold uppercase tracking-[.12em] !text-white transition-transform hover:-translate-y-0.5 xl:block">Contact</Link>
