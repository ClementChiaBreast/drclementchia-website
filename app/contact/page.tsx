...
export default function Contact() {
  return <section className="page-shell grid min-h-[720px] border-x hairline lg:grid-cols-[1fr_.8fr]">
    ...
    <div className="relative border-t hairline bg-[var(--paper)] p-6 sm:p-12 lg:border-l lg:border-t-0"><Orbit className="absolute right-6 top-6 w-32 text-[var(--ink)] opacity-15"/><div className="relative mt-16 grid gap-4 sm:grid-cols-3">{["For patients","For academics","For industry"].map((label)=><div key={label} className="border hairline bg-white/40 p-4"><span className="eyebrow text-[var(--coral)]">{label}</span></div>)}</div><ContactForm /></div>
  </section>;
}
